...
namespace Crud.DataBase.SqlDB
{
	public class EntityMapper
	{
		...
		public static IEnumerable<T> MapToEntities<T>(IDataReader dr)
		{
			...
			{
				// Our method will take in a single parameter, a IDataReader
				Type[] methodArgs = { typeof(IDataReader) };

				// The MapDR method will map a IDataReader row to an instance of type T
				...
				foreach (PropertyInfo pi in typeof(T).GetProperties())
				{
					...
					il.Emit(OpCodes.Callvirt, typeof(IDataReader).GetMethod("get_Item", new Type[] { typeof(string) }));
